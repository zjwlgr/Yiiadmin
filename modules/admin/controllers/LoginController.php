...
class LoginController extends Controller
{
    ...
    public function actionIndex() {
        $model = new ManagerInfo(['scenario' => 'login']);
        $model->attributes = Yii::$app->request->post('ManagerInfo');
        if($model->validate() && $model->load(Yii::$app->request->post())){
            Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;//json格式输出
            if($this->createAction('captcha')->validate($model->validates, false)){//验证码OK
                $result = $model->login($model->username);
                if($result['password'] != md5($model->password)){
